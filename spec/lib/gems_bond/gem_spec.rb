...
RSpec.describe GemsBond::Gem do
  ...
  it "handles gem data", :aggregate_failures do
    ...
    allow_any_instance_of(GemsBond::Fetchers::RubyGems).to receive(:last_version).and_return("0.4.3")
    expect(gem.last_version).to eq "0.4.3"

    allow_any_instance_of(GemsBond::Fetchers::RubyGems).to receive(:downloads_count).and_return(42)
    expect(gem.downloads_count).to eq 42

    allow_any_instance_of(GemsBond::Fetchers::RubyGems).to receive(:versions)
    ...
    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:login).and_return(true)
    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:set_repository).and_return(true)

    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:contributors_count).and_return(42)
    expect(gem.contributors_count).to eq 42

    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:stars_count).and_return(43)
    expect(gem.stars_count).to eq 43

    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:forks_count).and_return(44)
    expect(gem.forks_count).to eq 44

    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:last_commit_date).and_return(Date.new(2020, 1, 1))
    expect(gem.last_commit_date).to eq Date.new(2020, 1, 1)

    allow_any_instance_of(GemsBond::Fetchers::Github).to receive(:open_issues_count).and_return(45)
